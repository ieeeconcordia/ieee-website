...
export default function Event({ eventData }: any) {
  ...
  return (
    <RootLayout>
      ...
      <h1 className="font-lora font-bold text-center text-headline-l text-secondary pb-4">{eventData.name}</h1>
      <img src={`/api/uploads/${encodeURIComponent(eventData.image)}`} alt="" className="w-full max-h-80 object-cover rounded-lg mb-2" /></div>
      <div
